...
const props = withDefaults(defineProps<{
  ...
  nameKey?: string
  labelKey?: string
  labelFormatter?: (d: number | Date | string) => string
...
function formatValue(value: unknown): string {
  ...
  if (value && typeof (value as { toLocaleString?: () => string }).toLocaleString === 'function') {
    return (value as { toLocaleString: () => string }).toLocaleString()
  }
    ...
    <slot>
      ...
      <div class="grid gap-1.5">
        ...
        >
          ...
          <div :class="cn('flex min-w-0 flex-1 justify-between gap-4 leading-none', nestLabel ? 'items-end' : 'items-center')">
            ...
            <span v-if="value !== undefined && value !== null" class="text-foreground ml-auto shrink-0 font-mono font-medium tabular-nums">
              {{ formatValue(value) }}
            </span>
          </div>
        </div>
